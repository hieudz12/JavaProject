...
alter table HocVien
add Email nvarchar(50) not null default ''
...
alter table KhoaHoc
add NgayKetThuc Date not null default ''
...
go
ALTER TABLE ChuyenDe ADD  CONSTRAINT [DF_ChuyenDe_HocPhi]  DEFAULT ((0)) FOR [HocPhi]
GO
ALTER TABLE ChuyenDe ADD  CONSTRAINT [DF_ChuyenDe_ThoiLuong]  DEFAULT ((30)) FOR [ThoiLuong]
GO
ALTER TABLE ChuyenDe ADD  CONSTRAINT [DF_ChuyenDe_Hinh]  DEFAULT (N'chuyen-de.png') FOR [Hinh]
GO
ALTER TABLE ChuyenDe ADD  CONSTRAINT [DF_ChuyenDe_MoTa]  DEFAULT ('') FOR [MoTa]
GO
ALTER TABLE HocVien ADD  CONSTRAINT [DF_HocVien_Diem]  DEFAULT ((0)) FOR [Diem]
GO
ALTER TABLE KhoaHoc ADD  CONSTRAINT [DF_KhoaHoc_HocPhi]  DEFAULT ((0)) FOR [HocPhi]
GO
ALTER TABLE KhoaHoc ADD  CONSTRAINT [DF_KhoaHoc_ThoiLuong]  DEFAULT ((0)) FOR [ThoiLuong]
go
ALTER TABLE KhoaHoc ADD  CONSTRAINT DF_KhoaHoc_NgayTao DEFAULT (getdate()) FOR NgayTao
go
ALTER TABLE NguoiHoc ADD  CONSTRAINT [DF_NguoiHoc_GioiTinh]  DEFAULT ((0)) FOR GioiTinh
GO
ALTER TABLE NguoiHoc ADD  CONSTRAINT [DF_NguoiHoc_NgayDK]  DEFAULT (getdate()) FOR NgayDK
GO
ALTER TABLE NhanVien ADD  CONSTRAINT [DF_QuanTri_VaiTro]  DEFAULT ((0)) FOR VaiTro


go
alter table HocVien with check add constraint [FK_HocVien_KhoaHoc] foreign key(MaKH) references KhoaHoc(MaKH)
on update cascade 
on delete cascade
...
go
alter table HocVien with check add constraint [Fk_HocVien_NguoiHoc] foreign key(MaNH) references NguoiHoc(MaNH)
on update cascade
...
go
alter table KhoaHoc with check add constraint [FK_KhoaHoc_ChuyenDe] foreign key(MaCD) references ChuyenDe(MaCD)

go 
alter table KhoaHoc check constraint [FK_KhoaHoc_ChuyenDe]

go 
alter table KhoaHoc with check add constraint [FK_KhoaHoc_NhanVien] foreign key (MaNV) references NhanVien(MaNV)

go
alter table KhoaHoc check constraint [FK_KhoaHoc_NhanVien]

go
alter table NguoiHoc with check add constraint [FK_NguoiHoc_NhanVien] foreign key (MaNV) references NhanVien(MaNV)
...
GO
create or alter proc sp_DiemChuyenDe
as begin
	select
		TenCD ChuyenDe,
		COUNT(MaHV) SoHV,
...
INSERT KhoaHoc ([MaCD], [HocPhi], [ThoiLuong], [NgayKG], [GhiChu], [MaNV], [NgayTao]) VALUES ( N'JAV01', 250, 90, '2023/09/09', N'Lập trình Java cơ bản', N'TeoNV', CAST(0x0A410B00 AS Date))
...
select count(*) from HocVien where MaKH='1'
